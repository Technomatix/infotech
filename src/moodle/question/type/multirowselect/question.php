...
class qtype_multirowselect_question extends question_graded_automatically_with_countback
{
    public $rows;
    ...
    public function apply_attempt_state(question_attempt_step $step)
    {
        ...
        for ($i = 0; $i < count($this->order); $i++) {
            ...
            $a->number = -1;
            $a->optiontext = html_writer::span(get_string('deletedchoice', 'qtype_multirowselect'), 'notifyproblem');
     ...
     * @return bool
     */
    public function is_correct($value)
    {
        return $value > 0 && $value < getMaxAnswerValue($this->scoringmethod) + 1;
    }
    ...
    {
        return $this->is_gradable_response($response) ? '' : get_string('oneanswerperrow', 'qtype_multirowselect');
    }
     ...
     *
     * @return qtype_multirowselect_grading The grading object.
     */
    public function grading()
    {
        ...
        $gradingclass = 'qtype_multirowselect_grading_' . $type;

        require_once($CFG->dirroot . '/question/type/multirowselect/grading/' . $gradingclass . '.class.php');
